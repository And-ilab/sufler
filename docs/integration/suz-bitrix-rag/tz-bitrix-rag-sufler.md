...
sequenceDiagram
  ...
  Editor->>CMS: Публикация или изменение
...
| Требование | Штатно | Доработка | Обоснование выполнения доработки |
|------------|--------|-----------|----------------------------------|
| Чтение списка/статьи/разделов | REST [`iblock.element.list`](https://dev.1c-bitrix.ru/rest_help/iblock/elements/iblock_element_get_list.php), [`get`](https://dev.1c-bitrix.ru/rest_help/iblock/elements/iblock_element_get.php), [`getproperty`](https://dev.1c-bitrix.ru/rest_help/iblock/elements/iblock_element_getproperty.php); [`iblock.section.list`](https://dev.1c-bitrix.ru/rest_help/iblock/sections/iblock_section_get_list.php) / [`get`](https://dev.1c-bitrix.ru/rest_help/iblock/sections/iblock_section_get.php); обзор [REST](https://dev.1c-bitrix.ru/rest_help/) | Регламент ID инфоблока, коды свойств, сервисный токен (read) | Штатный REST не фиксирует банковский маппинг свойств и контур доступа к инфоблоку КЦ для RAG. |
| Реакция на Add/Update/Delete | [События ядра](https://dev.1c-bitrix.ru/api_help/main/events/index.php) / [инфоблока](https://dev.1c-bitrix.ru/api_help/iblock/events/index.php): `OnAfterIBlockElementAdd/Update/Delete` | Обработчики → payload + outbox | События ядра сами не собирают полный webhook-body модели B и не пишут outbox. |
| Push в суфлёр | Нет | HTTP POST webhook | Штатный REST не шлёт исходящие HTTP при сохранении элемента. |
| Надёжная доставка | Нет | Outbox: `pending` / `sent` / `failed`, retry | Нет гарантии доставки и повторной отправки во внешний контур суфлёра. |
| Уникальность сообщений | Нет | `event_id` (UUID) | Без устойчивого `event_id` суфлёр не может идемпотентно принять повтор webhook. |
| Статус draft/published/archived | Частично `ACTIVE` ([инфоблоки](https://dev.1c-bitrix.ru/api_help/iblock/index.php)) | Вычисление `status` по правилам банка | `ACTIVE` не равен бизнес-статусу draft/published/archived для КЦ. |
| Версии | История элемента ([инфоблоки](https://dev.1c-bitrix.ru/api_help/iblock/index.php)) | `version_id`, `is_current`, `event_type` | История инфоблока не отдаёт контрактные `version_id` / `is_current` / `event_type` для RAG. |
| Scope КЦ | Свойства в REST ([getproperty](https://dev.1c-bitrix.ru/rest_help/iblock/elements/iblock_element_getproperty.php)) | `visibility_scope`; вне scope — не publish | Свойства есть, но штатно нет правила «вне scope КЦ — не publish в суфлёр». |
| Permalink | Шаблоны сайта | Стабильный URL в payload | URL сайта зависит от шаблонов и не гарантирует стабильный permalink в webhook. |
| Checksum / changed_fields | Нет | sha256; массив кодов полей | Штатно нет признаков изменения для инкрементальной переиндексации RAG. |
| Инкремент хвоста | Частично `TIMESTAMP_X` в [`list`](https://dev.1c-bitrix.ru/rest_help/iblock/elements/iblock_element_get_list.php) | `GET …/changes?since=` из outbox | `TIMESTAMP_X` не даёт хвост доставленных событий outbox для reconciliation. |
| Retry по расписанию | [Агенты `CAgent`](https://dev.1c-bitrix.ru/api_help/main/reference/cagent/index.php) | Агент досылки `failed` + reconciliation | `CAgent` есть, но нет логики досылки `failed` webhook и сверки с суфлёром. |
| Защита канала | [OAuth](https://dev.1c-bitrix.ru/rest_help/oauth/index.php) входящего REST | TLS, allowlist, HMAC или mTLS исходящего POST | OAuth защищает входящий REST, а не исходящий POST webhook во внутренний контур. |
| Эксплуатация | Общие логи | Админ-журнал + replay | Общие логи Bitrix не дают журнал outbox и ручной replay того же `event_id`. |
